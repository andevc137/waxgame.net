...
router.get('/:account/:table', async (req, res) => {
    ...
    if (! ['bags', 'miners'].includes(table)) return res.status(400).send({msg: "Invalid Account."})

    await axios.post(url,
    {json: true, code: "m.federation", scope: "m.federation", table: table, lower_bound: account, upper_bound: account},
    {
        headers: {
